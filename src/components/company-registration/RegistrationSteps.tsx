  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${currentStep === 1 ? 'bg-primary text-white' : 'bg-gray-100 text-gray-400'}`}>
            <Upload className="h-5 w-5" />
          </div>
          <span className={`ml-3 text-sm font-medium ${currentStep === 1 ? 'text-primary' : 'text-gray-400'}`}>Documents</span>
        </div>
      </div>
    </div>
  );
